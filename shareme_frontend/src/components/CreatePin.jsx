...
const CreatePin = ({ user }) => {
  ...
  const savePin = () => {
    if(title && about && destination && imageAsset?._id && category){ // verify all data exsist
      const doc = {
        _type: 'pin',
        title: title,
        about: about,
        ...
        <div className="flex flex-1 flex-col gap-6 lg:pl-5 mt-5 w-full ">
          ...
          <div className="flex flex-col">
            ...
            <select
              ...
              className='outline-none w-4/5 text-base border-b-2 border-gray-200 cursor-pointer '
            >
              <option value='other' className='bg-white' >Select Category</option>
              {categories.map((category) => (<option className='text-base border-0 outline-none capitalize bg-white text-black' value={category.name} >{category.name}</option>))}
            </select>
          </div>
